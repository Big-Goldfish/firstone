...
<section>
                    ...
                    <div class="panel-body">
                        <form role="form" action="<?php echo U('');?>" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="exampleInputName" placeholder="请输入品牌名称" value='<?php echo ($rows["name"]); ?>'/>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputIntro">父级分类</label>
                                <select name="parent_id" id="parent_id">

                                    <?php if(is_array($row)): $i = 0; $__LIST__ = $row;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><option value="<?php echo ($row["id"]); ?>"><?php echo str_repeat('&nbsp;&nbsp;',$row['level']); echo ($row["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="exampleInputIntro">分类简介</label>
                                <textarea class="form-control " id="exampleInputIntro" name="intro" placeholder="请输入品牌简介"><?php echo ($rows["intro"]); ?></textarea>
                            </div>

                            <?php if(isset($rows)): ?><input type="hidden" name="id" value="<?php echo ($rows["id"]); ?>">
    ...
    <script type="text/javascript">
        $(function(){
            $('#parent_id').val(<?php echo ($rows["parent_id"]); ?>)
        })
    </script>
