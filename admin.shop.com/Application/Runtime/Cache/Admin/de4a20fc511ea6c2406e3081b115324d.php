...
<section>
    ...
    <script type="text/javascript">
        $('#parent_id').val(<?php echo ($lists["parent_id"]); ?>);
        console.log(<?php echo ($lists["parent_id"]); ?>)
                    ...
                    <div class="panel-body">
                        <form role="form" action="<?php echo U('');?>" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" placeholder="请输入权限名称" value='<?php echo ($lists["name"]); ?>'/>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputName">权限路径</label>
                                <input type="text" name="path" class="form-control"  placeholder="请输入权限路径" value='<?php echo ($lists["path"]); ?>'/>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputIntro">权限分级</label>
                                <select name="parent_id" id="parent_id">
                                    <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><option value="<?php echo ($row["id"]); ?>"><?php echo str_repeat('&nbsp;&nbsp;',$row['level']); echo ($row["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="exampleInputIntro">分类简介</label>
                                <textarea class="form-control " name="intro" placeholder="请输入权限简介"><?php echo ($lists["intro"]); ?></textarea>
                            </div>

                            <?php if(isset($lists)): ?><input type="hidden" name="id" value="<?php echo ($lists["id"]); ?>">
