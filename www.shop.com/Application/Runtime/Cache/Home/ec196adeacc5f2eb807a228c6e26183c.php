			...
			<div class="panel">
				...
				<table class="cargo">
					...
					<?php if(is_array($goodslist)): $i = 0; $__LIST__ = $goodslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goodlist): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><img src="<?php echo ($goodlist["logo"]); ?>" /></td>
							<td><ul><li><?php echo ($goodlist["content"]); ?> </li></ul></td>
							<td>￥：<?php echo ($goodlist["shop_price"]); ?></td>
							<td><button class="decrease">-</button><input class="amount" type="text" value="<?php echo ($goodlist["amount"]); ?>"/><button class="increase">+</button></td>
					...
					<tfoot>
						<tr>
							...
							<td>已选择商品 <span class="count"><?php echo ($sum_total); ?></span> 件</td>
							<td colspan="2">合计（不含运费）：<span class="sum">￥<?php echo ($sum_price); ?></span>　<a href="<?php echo U('Order/create');?>"><button class="submit">结算</button></a></td>
						</tr>
					</tfoot>
				</table>
			</div>
